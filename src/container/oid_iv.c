		   ...
		   d_sg_list_t *src, int ref_rc, void **_priv)
{
	...
	D_DEBUG(DB_MD, "%u: ON REFRESH: num_oids = %zu, REF_RC = " DF_RC " ENTRY rank %u Ptr = %p\n",
		dss_self_rank(), num_oids, DP_RC(ref_rc), entry->current_req_rank, entry->current_req_ptr);
