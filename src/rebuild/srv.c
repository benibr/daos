...
update_and_warn_for_slow_engines(struct rebuild_global_pool_tracker *rgt)
{
	...
	D_INFO(DF_RB ": s_done=%s, s_ct=%d, p_done=%s, p_ct=%d, servers=%d, d_ct=%d, almost=%s\n",
	       DP_RB_RGT(rgt), scan_gl ? "yes" : "no", scan_ct, pull_gl ? "yes" : "no", pull_ct,
		   rgt->rgt_servers_number, done_ct,
	...
	if (is_rebuild_phase_mostly_done(done_ct, rgt->rgt_servers_number)) {
		const char *phase = scan_gl ? "pull" : "scan";
		...
		if ((!scan_gl && now > rgt->rgt_scan_warn_deadline_ts) ||
		    (!pull_gl && now > rgt->rgt_pull_warn_deadline_ts)) {
			int wait_ct = rgt->rgt_servers_number - done_ct;

			D_WARN(DF_RB ": %s hung? waiting for %d/%d engines:\n",
			       DP_RB_RGT(rgt), phase, wait_ct, rgt->rgt_servers_number);
			warned = true;
		}
